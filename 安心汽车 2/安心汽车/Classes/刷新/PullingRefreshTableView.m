...
#define kPRMargin 16.f
#define kPRLabelHeight 20.f
#define kPRLabelWidth 100.f
#define kPRArrowWidth 38.f  
#define kPRArrowHeight 38.f

#define kTextColor [UIColor colorWithRed:0.0/255.0 green:0.0/255.0 blue:0.0/255.0 alpha:0.5]
...
- (void)updateRefreshDate :(NSDate *)date isFooter:(BOOL) footer;
- (void)layouts;
@end
...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top {
    ...
    if (self) {
        ...
        _stateLabel.font = ft;
        //_stateLabel.textColor = RGBA(254, 114, 0, 1);
        _stateLabel.textColor = RGBACOLOR(1, 254, 114, 0);
        ...
        _dateLabel.font = ft;
        _dateLabel.textColor = RGBA(254, 114, 0, 1);
...
//            [_activityView startAnimating];
            ...
            if (_arrow) {
                [_arrow removeAllAnimations];
            }
            
            CABasicAnimation *animationUp  = [CABasicAnimation animationWithKeyPath:@"position"];
            animationUp.fromValue =  [NSValue valueWithCGPoint:_arrow.position];
            CGPoint toPointUp = _arrow.position;
            toPointUp.y += 15;
            animationUp.toValue = [NSValue valueWithCGPoint:toPointUp];
            
            CABasicAnimation *animationDown  = [CABasicAnimation animationWithKeyPath:@"position"];
            animationDown.fromValue =  [NSValue valueWithCGPoint:_arrow.position];
            CGPoint toPointDown = _arrow.position;
            toPointDown.y -= 15;
            animationDown.toValue = [NSValue valueWithCGPoint:toPointDown];
            
            CAAnimationGroup *animationGroup = [CAAnimationGroup animation];
            animationGroup.duration = 0.25f;
            animationGroup.autoreverses = YES;   //是否重播，原动画的倒播
            animationGroup.repeatCount = NSNotFound;
            [animationGroup setAnimations:[NSArray arrayWithObjects:animationUp, animationDown, nil]];
            
            [_arrow addAnimation:animationGroup forKey:@"animationGroup"];
...
//            [_arrow removeAnimationForKey:@"animationGroup"];
...
- (void)tableViewDidFinishedLoadingWithMessage:(NSString *)msg{
    ...
    if (_headerView.loading) {
        ...
        [_headerView updateRefreshDate:date isFooter:FALSE];
    ...
    //    if (_footerView.state == kPRStateLoading) {
        ...
        [_footerView updateRefreshDate:date isFooter:TRUE];
...
- (void)flashMessage:(NSString *)msg{
    ...
    __block CGRect rect = CGRectMake(0, self.contentOffset.y - 20, self.bounds.size.width, 26);
    if (_msgLabel == nil) {
        ...
        _msgLabel.backgroundColor = RGBACOLOR(4, 175, 200, 1);
        _msgLabel.textColor = [UIColor whiteColor];
        _msgLabel.layer.cornerRadius = 5.0f;
        [_msgLabel.layer setMasksToBounds:TRUE];
    ...
    [UIView animateWithDuration:.5f animations:^{
        ...
        rect.origin.y -= 26;
        [UIView animateWithDuration:.5f delay:1.2f options:UIViewAnimationOptionCurveLinear animations:^{
            _msgLabel.frame = rect;
        } completion:^(BOOL finished){
